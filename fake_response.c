...
{
  if (argc != 8) {
        printf("Usage: ./get src_ip dst_ip seq ack src_port dst_port content\n");
        return;
  }
  ...
  char* response1 = "HTTP/1.1 200 OK\r\nServer: nginx/1.4.4\r\nDate: Sun, 14 Jun 2015 13:41:03 GMT\r\nContent-Type: text/html\r\nContent-Length: ";
  char response2[] = "";
  sprintf(response2, "%d", strlen(argv[7]));
  char* response3 = "\r\nLast-Modified: Sun, 14 Jun 2015 11:46:29 GMT\r\nConnection: keep-alive\r\nETag: \"557d6995-6\"\r\nAccept-Ranges: bytes\r\n\r\n";
  char* response4 = argv[7];

  strcpy(payload, str_contact(str_contact(response1, response2), str_contact(response3, response4)));
  ...
  tcphdr.th_sport = htons (atoi(argv[5]));
  
  // Destination port number (16 bits)
  tcphdr.th_dport = htons (atoi(argv[6]));

  // Sequence number (32 bits)
  tcphdr.th_seq = htonl (atol(argv[3]));
  
  // Acknowledgement number (32 bits): 0 in first packet of SYN/ACK process
  tcphdr.th_ack = htonl (atol(argv[4]));
